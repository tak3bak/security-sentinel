...
class sfp_dehashed(SpiderFootPlugin):
    ...
    def query(self, event, per_page, start):
        ...
        token = (
            base64.b64encode(
                self.opts["api_key_username"].encode("utf8")
                + ":".encode("utf-8")
                + self.opts["api_key"].encode("utf-8")
            )
        ).decode("utf-8")
    ...
    def handleEvent(self, event):
        ...
        if self.opts["api_key"] == "" or self.opts["api_key_username"] == "":
            self.error(
                "You enabled sfp_dehashed but did not set an API key/API Key Username!"
            )
            self.errorState = True
            return
                ...
                if eventName == "EMAILADDR":
                    if email == eventData:
                        ...
                        if passwordHash:
                            evt = SpiderFootEvent(
                                "HASH_COMPROMISED",
                                f"{email}:{passwordHash} [{leakSource}]",
                                self.__name__,
                                event,
                            )
                            self.notifyListeners(evt)
