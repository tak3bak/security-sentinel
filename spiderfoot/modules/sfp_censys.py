...
class sfp_censys(SpiderFootPlugin):
    ...
    def queryHosts(self, qry):
        secret = (
            self.opts["censys_api_key_uid"] + ":" + self.opts["censys_api_key_secret"]
        )
    ...
    def handleEvent(self, event):
        ...
        if (
            self.opts["censys_api_key_uid"] == ""
            or self.opts["censys_api_key_secret"] == ""
        ):
            self.error(
        ...
        for addr in qrylist:
            ...
            try:
                ...
                if services:
                    ...
                    for banner in set(tcp_banners):
                        evt = SpiderFootEvent(
                            "TCP_PORT_OPEN_BANNER", str(banner), self.__name__, pevent
                        )
                        self.notifyListeners(evt)
            except Exception as e:
                self.error(
            ...
            try:
                ...
                if autonomous_system:
                    ...
                    if asn:
                        e = SpiderFootEvent(
                            "BGP_AS_MEMBER", str(asn), self.__name__, pevent
                        )
                        self.notifyListeners(e)
