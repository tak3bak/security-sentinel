...
class sfp_numverify(SpiderFootPlugin):
    ...
    def query(self, qry):
        ...
        params = {
            "number": number.encode("raw_unicode_escape").decode(
        ...
        res = self.sf.fetchUrl(
            "http://apilayer.net/api/validate?" + urllib.parse.urlencode(params),
    ...
    def handleEvent(self, event):
        ...
        if data.get("carrier"):
            evt = SpiderFootEvent(
                "PROVIDER_TELCO", data.get("carrier"), self.__name__, event
            )
            self.notifyListeners(evt)
        else:
