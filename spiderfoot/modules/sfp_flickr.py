...
class sfp_flickr(SpiderFootPlugin):
    ...
    def query(self, qry, api_key, page=1, per_page=200):
        ...
        res = self.sf.fetchUrl(
            "https://api.flickr.com/services/rest?" + urllib.parse.urlencode(params),
            useragent=self.opts["_useragent"],
            timeout=self.opts["_fetchtimeout"],
        )
    ...
    def handleEvent(self, event):
        ...
        while page <= pages:
            ...
            if urlparse(photos).netloc.lower() == "max_allowed_pages" or urlparse(
                photos
            ).netloc.lower().endswith(".max_allowed_pages"):
            ...
            for photo in photos.get("photo", list()):
                ...
                for email in emails:
                    ...
                    if not self.getTarget().matches(
                        mail_domain, includeChildren=True, includeParents=True
                    ):
