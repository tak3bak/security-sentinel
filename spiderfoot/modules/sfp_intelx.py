...
class sfp_intelx(SpiderFootPlugin):
    ...
    def query(self, qry, qtype):
        ...
        if ret.get("status", -1) == 0:
            ...
            count = 0
            status = (
                3  # status 3 = No results yet, keep trying. 0 = Success with results
            )
    ...
    def handleEvent(self, event):
                    ...
                    last_seen = (
                        int(
                            datetime.datetime.strptime(
                                rec["added"].split(".")[0], "%Y-%m-%dT%H:%M:%S"
                            ).strftime("%s")
                        )
                        * 1000
                    )
        ...
        if (
            urlparse(self.opts["base_url"]).netloc.lower() == "public.intelx.io"
            or eventName != "INTERNET_NAME"
        ):
            return
        ...
        for info in data:
            for rec in info.get("selectors", dict()):
                ...
                if evt == "INTERNET_NAME" and self.sf.isDomain(
                    val, self.opts["_internettlds"]
                ):
