...
class sfp_securitytrails(SpiderFootPlugin):
    ...
    # Default options
    opts = {"api_key": "", "verify": True, "cohostsamedomain": False, "maxcohost": 100}
    ...
    def handleEvent(self, event):
        ...
        if eventName in ["IP_ADDRESS", "IPV6_ADDRESS", "NETLBLOCK_OWNER"]:
            ...
            if rec is not None:
                for r in rec:
                    if urlparse(r).netloc.lower() == "host_provider" or urlparse(
                        r
                    ).netloc.lower().endswith(".host_provider"):
                        ...
                        for dat in r["host_provider"]:
                            if dat in hosters:
                                continue
                            e = SpiderFootEvent(
                                "PROVIDER_HOSTING", dat, self.__name__, event
                            )
                            self.notifyListeners(e)
                            hosters.append(dat)
                    if urlparse(r).netloc.lower() == "hostname" or urlparse(
                        r
                    ).netloc.lower().endswith(".hostname"):
                        ...
                        if h not in myres and h != ip:
                            if self.opts["verify"] and not self.sf.validateIP(h, ip):
                                self.debug(
                                    "Host " + h + " no longer resolves to our IP."
                                )
                                continue
            ...
            if rec is not None:
                for h in rec:
                    ...
                    if h.lower() not in myres:
                        ...
                        continue
                    e = SpiderFootEvent(
                        "INTERNET_NAME", h + "." + domain, self.__name__, event
                    )
                    self.notifyListeners(e)
