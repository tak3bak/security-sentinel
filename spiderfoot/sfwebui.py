...
class SpiderFootWebUi:
    ...
    def buildExcel(
        self: "SpiderFootWebUi", data: list, columnNames: list, sheetNameIndex: int = 0
    ) -> str:
    ...
    def startscan(
        ...
        if len(modlist) == 0 and usecase:
            for mod in self.config["__modules__"]:
                if (
                    usecase == "all"
                    or usecase in self.config["__modules__"][mod]["group"]
                ):
                    modlist.append(mod)
