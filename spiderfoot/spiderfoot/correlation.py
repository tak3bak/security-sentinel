...
class SpiderFootCorrelator:
        ...
        for chunk in event_chunks:
            ...
            for row in child_data:
                events[row[9]]["child"].append(
                    {"type": row[4], "data": row[1], "module": row[3], "id": row[8]}
                )
    ...
    def event_keep(
        self, event: dict, field: str, patterns: str, patterntype: str
    ) -> bool:
        """Keep event field.
    ...
    def analysis_threshold(self, rule: dict, buckets: dict) -> None:
        ...
        for bucket in list(buckets.keys()):
            ...
            if not rule.get("count_unique_only"):
                for v in countmap:
                    if countmap[v] >= rule.get("minimum", 0) and countmap[
                        v
            ...
            if uniques < rule.get("minimum", 0) or uniques > rule.get(
                "maximum", 999999999
            ):
                del buckets[bucket]
