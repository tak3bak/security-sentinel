...
class TestSpiderFootHelpers(unittest.TestCase):
    ...
    def test_urlRelativeToAbsolute_argument_url_relative_path_should_return_an_absolute_path(
        self,
    ):
        relative_url = SpiderFootHelpers.urlRelativeToAbsolute(
            "/somewhere/else/../../path?param=value#fragment"
        )
    ...
    def test_dictionaryWordsFromWordlists_argument_wordlists_missing_wordlist_should_raise_IOError(
        self,
    ):
    ...
    def test_humanNamesFromWordlists_argument_wordlists_missing_wordlist_should_raise_IOError(
        self,
    ):
        with self.assertRaises(IOError):
    ...
    def test_usernamesFromWordlists_argument_wordlists_missing_wordlist_should_raise_IOError(
        self,
    ):
        with self.assertRaises(IOError):
    ...
    def test_extractLinksFromHtml_argument_data_not_containing_links_should_return_an_empty_dict(
        self,
    ):
        parse_links = SpiderFootHelpers.extractLinksFromHtml(
            "url", "<html>example html content</html>", "domains"
        )
        self.assertIsInstance(parse_links, dict)
        self.assertFalse(parse_links)

    def test_extractLinksFromHtml_argument_data_containing_malformed_html_with_links_should_return_a_dict_of_urls(
        self,
    ):
        ...
        self.assertEqual(
            parse_links,
            {
                "http://spiderfoot.net/xmlrpc.php": {
                    "source": url,
                    "original": "http://spiderfoot.net/xmlrpc.php",
                },
                "http://spiderfoot.net/path": {
                    "source": url,
                    "original": "http://spiderfoot.net/path",
                },
                "http://spiderfoot.net/relative-path": {
                    "source": url,
    ...
    def test_extractLinksFromHtml_invalid_url_should_raise_TypeError(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                with self.assertRaises(TypeError):
                    SpiderFootHelpers.extractLinksFromHtml(
                        invalid_type, "example html content", "domains"
                    )
    ...
    def test_extractLinksFromHtml_invalid_domains_should_return_a_dict(self):
        ...
        for invalid_type in invalid_types:
            with self.subTest(invalid_type=invalid_type):
                parse_links = SpiderFootHelpers.extractLinksFromHtml(
                    "url", "example html content", invalid_type
                )
                self.assertIsInstance(parse_links, dict)
