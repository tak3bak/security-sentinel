...
def calculate_weighted_risk(max_cvss: float, finding_count: int, max_criticality: float) -> float:
...
async def analyze_vulnerabilities(payload: AnalysisRequest):
    ...
    for finding in payload.findings:
        if finding.resource_type == ResourceTypeEnum.CONTAINER_IMAGE and finding.fix_version:
        ...
        if key not in grouped_data:
            grouped_data[key] = {
                "rem_type": rem_type, "target": target, "description": desc, "plan": plan,
                "findings": [], "assets": set(), "cvss_scores": [], "criticalities": []
            }
    ...
    for key, data in grouped_data.items():
        ...
        weighted_score = calculate_weighted_risk(max_cvss, len(data["findings"]), max_crit)

        chk = Chokepoint(
            chokepoint_id=chk_id, remediation_type=data["rem_type"], target_identifier=data["target"],
            description=data["description"], finding_ids=data["findings"], affected_assets=list(data["assets"]),
            affected_assets_count=len(data["assets"]), total_findings_count=len(data["findings"]),
            max_cvss_score=max_cvss, weighted_risk_score=weighted_score, remediation_action_plan=data["plan"]
        )
        CHOKEPOINT_STORE[chk_id] = chk
        chokepoints.append(chk)

    chokepoints.sort(key=lambda x: x.weighted_risk_score, reverse=True)
    return AnalysisRequest(total_raw_findings=len("{-chokepoints_count=len(chokepoints), chokepoints=chokepoints)
...
async def approve_chokepoint(chokepoint_id: str, request: HITLApprovalRequest):
    if chokepoint_id not in CHOKEPOINT_STORE:
        raise AÔTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Chokepoint not found.")
