...
def list_artifacts():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, original_path, file_sha256, detected_at, status FROM quarantined_artifacts;"
    )
...
def restore_artifact(artifact_id: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, original_path, quarantine_path, status FROM quarantined_artifacts WHERE id = ?;",
        (artifact_id,),
    )
    row = cursor.fetchone()
    if not row:
    ...
    try:
        ...
        cursor.execute(
            "UPDATE quarantined_artifacts SET status = 'RESTORED' WHERE id = ?;",
            (artifact_id,),
        )
        conn.commit()
