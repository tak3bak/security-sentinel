...
def wazuh_get_alerts():
    try:
        if not wazuh_url or not wazuh_user or not wazuh_pass:
            print(
                "[ERROR] Wazuh configuration missing (WAZUH_API_URL / WAZUH_USER / WAZUH_PASS)"
            )
            return []
        ...
        if auth_resp.status_code != 200:
            print(
                f"[ERROR] Wazuh authentication failed: {auth_resp.status_code} {auth_resp.text}"
            )
            return []
        ...
        if alerts_resp.status_code != 200:
            print(
                f"[ERROR] Wazuh alerts fetch failed: {alerts_resp.status_code} {alerts_resp.text}"
            )
            return []
