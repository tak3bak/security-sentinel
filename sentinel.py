...
class SentinelHandler(FileSystemEventHandler):
    ...
    def process_threat(self, file_path):
        ...
        with self.lock:
            now = time.time()
            if filename in self.processed_files and (
                now - self.processed_files[filename] < 1.0
            ):
                return
            self.processed_files[filename] = now

        if not filename.endswith((".malware", ".malwarere")) and not os.path.isdir(
            file_path
        ):
        ...
        logging.warning(
            f"{mode_tag} [THREAT INDICATOR MATCHED] File discovered: {filename} at {file_path}"
        )
        ...
        threading.Thread(
            target=self.enrich_threat_data, args=(ip_target, mode_tag), daemon=True
        ).start()

    def quarantine_file(self, src_path, filename, mode_tag):
        os.makedirs(QUARANTINE_DIR, exist_ok=True)
        dest_path = os.path.join(
            QUARANTINE_DIR, f"isolated_{int(time.time())}_{filename}"
        )

        if DRY_RUN_MODE:
            logging.info(
                f"{mode_tag} [TELEMETRY COMPLIANCE] Simulation asset tracking. Would safely move: {src_path} -> {dest_path}"
            )
            return
        ...
        try:
            if not os.access(src_path, os.W_OK):
                logging.error(
                    f"[QUARANTINE EXCEPTION] Insufficient engine execution privileges on path: {src_path}"
                )
                return
            shutil.move(src_path, dest_path)
            logging.info(
                f"[QUARANTINE SUCCESS] Asset isolated securely under container: {dest_path}"
            )
        ...
        if SHODAN_API_KEY and SHODAN_API_KEY != "your_shodan_api_key_here":
            try:
                ...
                if res.status_code == 200:
                    data = res.json()
                    logging.info(
                        f"{mode_tag} [SHODAN MATCH] ISP: {data.get('isp', 'Unknown')} | Open Ports: {data.get('ports', [])}"
                    )
            except Exception as e:
                logging.error(
                    f"{mode_tag} [SHODAN FAULT] Connection failed: {type(e).__name__}"
                )
        ...
        if os.path.exists(sf_cli):
            try:
                ...
                subprocess.Popen(
                    ...
                    cwd=SPIDERFOOT_PATH_ENV,
                )
                logging.info(
                    f"{mode_tag} [SPIDERFOOT ENRICHMENT] Dispatched background OSINT workflow: {scan_name}"
                )
            except Exception as e:
                logging.error(
                    f"{mode_tag} [SPIDERFOOT FAULT] Failed to call CLI routing layers: {e}"
                )
...
if __name__ == "__main__":
    ...
    if not verify_beta_license(LICENSE_KEY):
        logging.critical(
            "CRITICAL AUTH EXCEPTION: Invalid, expired, or missing Nomadik Beta License Key."
        )
        logging.critical(
            "Please confirm your $49/month subscription parameters inside your local .env configuration."
        )
        sys.exit(1)
    ...
    if DRY_RUN_MODE:
        logging.info(
            "👉 DEFENSE LAYER CONFIGURATION: DRY_RUN mode is ACTIVE. System modifications will be safely simulated."
        )
    else:
        logging.info(
            "🛡️ DEFENSE LAYER CONFIGURATION: ACTIVE ENFORCEMENT is enabled. Sudo containment policies are live."
        )
    ...
    for path in SYSTEM_WATCH_PATHS:
        if os.path.exists(path):
            try:
                ...
                logging.warning(
                    f"Could not hook file system observer onto path {path}: {e}"
                )

    if watches == 0:
        logging.critical(
            "Engine runtime failed: No valid monitoring paths could be initialized on this host."
        )
        sys.exit(1)
