    ...
    def handleEvent(self, event):
        ...
        if eventName == "TARGET_WEB_CONTENT":
            ...
            for m in matches:
                ...
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Google Analytics: " + m, self.__name__, event
                )
            ...
            for m in matches:
                ...
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", "Google AdSense: " + m, self.__name__, event
                )
                evt.moduleDataSource = datasource
                self.notifyListeners(evt)
            ...
            for m in set(matches):
                ...
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID", f"Google Tag Manager: {m}", self.__name__, event
                )
                evt.moduleDataSource = datasource
                self.notifyListeners(evt)
            ...
            # Quantcast
            if urlparse(eventData).netloc.lower() == "_qevents.push" or urlparse(
                eventData
            ).netloc.lower().endswith("._qevents.push"):
                matches = re.findall(
                    r"\bqacct:\"(p-[a-z0-9]+)\"", eventData, re.IGNORECASE
                )
                for m in matches:
                    self.debug("Quantcast match: " + m)
                    evt = SpiderFootEvent(
                        "WEB_ANALYTICS_ID", "Quantcast: " + m, self.__name__, event
                    )
                    evt.moduleDataSource = datasource
                    self.notifyListeners(evt)
        ...
        if eventName == "DNS_TEXT":
            ...
            matches = re.findall(
                r"adobe-sign-verification=([a-f0-9]{32})$",
                eventData.strip(),
                re.IGNORECASE,
            )
            for m in matches:
            ...
            matches = re.findall(
                r"stripe-verification=([a-f0-9]{64})$", eventData.strip(), re.IGNORECASE
            )
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID",
            ...
            matches = re.findall(
                r"detectify-verification=([a-f0-9]{32})$",
                eventData.strip(),
                re.IGNORECASE,
            )
            for m in matches:
            ...
            matches = re.findall(
                r"drift-verification=([a-f0-9]{64})$", eventData.strip(), re.IGNORECASE
            )
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID",
            ...
            matches = re.findall(
                r"ahrefs-site-verification_([a-f0-9]{64})$",
                eventData.strip(),
                re.IGNORECASE,
            )
            for m in matches:
            ...
            matches = re.findall(
                r"ZOOM_verify_([a-z0-9\-\+\/_=]{22})$", eventData.strip(), re.IGNORECASE
            )
            for m in matches:
                evt = SpiderFootEvent(
                    "WEB_ANALYTICS_ID",
            ...
            matches = re.findall(
                r"mailru-verification: ([a-z0-9]{16})$",
                eventData.strip(),
                re.IGNORECASE,
            )
            for m in matches:
            ...
            matches = re.findall(
                r"yandex-verification: ([a-z0-9]{16})$",
                eventData.strip(),
                re.IGNORECASE,
            )
            for m in matches:
            ...
            matches = re.findall(
                r"brave-ledger-verification=([a-z0-9]+)$",
                eventData.strip(),
                re.IGNORECASE,
            )
            for m in matches:
            ...
            matches = re.findall(
                r"cisco-ci-domain-verification=([a-f0-9]+)$",
                eventData.strip(),
                re.IGNORECASE,
            )
            for m in matches:
