...
class sfp_trashpanda(SpiderFootPlugin):
    ...
    meta = {
        ...
        "dataSource": {
            ...
            "apiKeyInstructions": ["Follow the guide at https://got-hacked.wtf/"],
    ...
    def handleEvent(self, event):
        ...
        for row in data:
            evt = SpiderFootEvent(
                "PASSWORD_COMPROMISED",
                f"{row.get('email')}:{row.get('password')} [{row.get('paste')}]",
                self.__name__,
                event,
            )
        ...
        for leaksiteUrl in leaksiteUrls:
            try:
                ...
                evt = SpiderFootEvent(
                    "LEAKSITE_CONTENT", res["content"], self.__name__, evt
                )
                self.notifyListeners(evt)
