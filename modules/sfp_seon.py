...
class sfp_seon(SpiderFootPlugin):
    ...
    def query(self, qry, eventName):
        ...
        res = self.sf.fetchUrl(
            queryString, headers=headers, timeout=15, useragent=self.opts["_useragent"]
        )
        ...
        if eventName in ["IP_ADDRESS", "IPV6_ADDRESS"]:
                    ...
                    for site in socialMediaList:
                        if resultSet.get("account_details").get(site):
                            if resultSet.get("account_details").get(site).get("url"):
                                evt = SpiderFootEvent(
                                    "SOCIAL_MEDIA",
                                    f"{site}: <SFURL>{resultSet.get('account_details').get(site).get('url')}</SFURL>",
                                    self.__name__,
                                    event,
                                )
                                self.notifyListeners(evt)
                            elif (
                                resultSet.get("account_details")
                                .get(site)
                                .get("registered")
                            ):
                                evt = SpiderFootEvent(
                                    "SOCIAL_MEDIA",
                                    f"Registered on {site}",
                                    self.__name__,
                                    event,
                                )
                                self.notifyListeners(evt)
            ...
            if resultSet:
                ...
                if resultSet.get("type"):
                    evt = SpiderFootEvent(
                        "PHONE_NUMBER_TYPE", resultSet.get("type"), self.__name__, event
                    )
                    self.notifyListeners(evt)
                    dataFound = True
                ...
                if resultSet.get("carrier"):
                    evt = SpiderFootEvent(
                        "PROVIDER_TELCO", resultSet.get("carrier"), self.__name__, event
                    )
                    self.notifyListeners(evt)
                    dataFound = True
