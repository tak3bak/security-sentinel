...
class sfp_riskiq(SpiderFootPlugin):
    ...
    def query(self, qry, qtype, opts=dict()):
        ...
        res = self.sf.fetchUrl(
            url, timeout=30, useragent="SpiderFoot", headers=headers, postData=post
        )
        ...
        if eventName == "EMAILADDR":
            ...
            for r in ret:
                if not eventData.endswith("@" + r["domain"]):
                    ...
                    if t == "AFFILIATE_INTERNET_NAME" and self.sf.isDomain(
                        r["domain"], self.opts["_internettlds"]
                    ):
                        evt = SpiderFootEvent(
        ...
        if eventName in ["IP_ADDRESS", "INTERNET_NAME", "DOMAIN_NAME"]:
            ...
            if eventName == "IP_ADDRESS":
                for r in ret:
                    ...
                    if (
                        not self.getTarget().matches(r["focusPoint"])
                        and "*" not in r["focusPoint"]
                    ):
                        # We found a co-host
