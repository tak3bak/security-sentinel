...
class sfp_pulsedive(SpiderFootPlugin):
    ...
    def query(self, qry):
        params = {
            "indicator": qry.encode("raw_unicode_escape").decode(
        ...
        if res["code"] == "403":
            self.error(
                "Pulsedive API key seems to have been rejected or you have exceeded usage limits for the month."
            )
            self.errorState = True
            return None
        ...
        for addr in qrylist:
            ...
            if attributes:
                ...
                if ports:
                    for p in ports:
                        e = SpiderFootEvent(
                            "TCP_PORT_OPEN", addr + ":" + p, self.__name__, event
                        )
            ...
            for result in threats:
                ...
                if tid:
                    descr += (
                        "\n<SFURL>https://pulsedive.com/indicator/?iid="
                        + tid
                        + "</SFURL>"
                    )
