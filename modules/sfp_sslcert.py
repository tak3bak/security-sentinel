...
class sfp_sslcert(SpiderFootPlugin):
    ...
    # Default options
    opts = {"tryhttp": True, "verify": True, "ssltimeout": 10, "certexpiringdays": 30}
    ...
    def handleEvent(self, event):
        ...
        if eventName == "LINKED_URL_INTERNAL":
            if (
                not eventData.lower().startswith("https://")
                and not self.opts["tryhttp"]
            ):
                return
        ...
        if eventName in ["INTERNET_NAME", "IP_ADDRESS"]:
            evt = SpiderFootEvent(
                "TCP_PORT_OPEN", fqdn + ":" + str(port), self.__name__, event
            )
