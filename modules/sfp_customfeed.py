...
class sfp_customfeed(SpiderFootPlugin):
    ...
    # Default options
    opts = {"checkaffiliates": True, "checkcohosts": True, "url": "", "cacheperiod": 0}
        ...
        for check in list(malchecks.keys()):
            ...
            if replaceme_id == cid:
                ...
                if "regex" not in malchecks[check]:
                    for line in data["content"].split("\n"):
                        if line == target or (
                            targetType == "domain" and line == targetDom
                        ):
                            self.debug(
                                target
                                + "/"
                    ...
                    try:
                        ...
                        for line in data["content"].split("\n"):
                            if (
                                targetType == "domain"
                                and re.match(rxDom, line, re.IGNORECASE)
                            ) or re.match(rxTgt, line, re.IGNORECASE):
                                self.debug(
                                    target
                                    + "/"
                                    + targetDom
    ...
    def handleEvent(self, event):
        ...
        if eventName == "CO_HOSTED_SITE" and not self.opts.get("checkcohosts", False):
            return
        if eventName == "AFFILIATE_IPADDR" and not self.opts.get(
