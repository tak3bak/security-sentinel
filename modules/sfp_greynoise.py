...
class sfp_greynoise(SpiderFootPlugin):
    ...
    def handleEvent(self, event):
                ...
                if rec.get("seen", None):
                    ...
                    if rec.get("metadata") and eventName == "IP_ADDRESS":
                        ...
                        if met.get("os", "unknown") != "unknown":
                            e = SpiderFootEvent(
                                "OPERATING_SYSTEM", met.get("os"), self.__name__, event
                            )
                            self.notifyListeners(e)
                        e = SpiderFootEvent(
                    ...
                    if rec.get("classification"):
                        ...
                        if rec.get("raw_data") and not (
                            rec.get("tags") or ret.get("cve")
                        ):
        ...
        if urlparse(ret).netloc.lower() == "seen" or urlparse(
            ret
        ).netloc.lower().endswith(".seen"):
