...
class sfp_dnsresolve(SpiderFootPlugin):
    ...
    def enrichTarget(self, target):
        ...
        for host in ret:
            ...
            if self.sf.validIP(host):
                ...
                if idnahost != host:
                    target.setAlias(
                        idnahost.decode("ascii", errors="replace"), "INTERNET_NAME"
                    )
    ...
    def handleEvent(self, event) -> None:
        ...
        if (
            eventData.endswith(".ipv6-literal.net")
            and eventName == "AFFILIATE_INTERNET_NAME"
        ):
            ipv6 = (
                eventData.split(".ipv6-literal.net")[0]
                .replace("-", ":")
                .replace("s", "%")
                .split("%")[0]
            )
        ...
        if (
            eventData.endswith(".in-addr.arpa")
            and eventName == "AFFILIATE_INTERNET_NAME"
        ):
    ...
    def processDomain(self, domainName, parentEvent, affil=False, host=None) -> None:
        ...
        if affil:
            domevt = SpiderFootEvent(
                "AFFILIATE_DOMAIN_NAME", domainName, self.__name__, parentEvent
            )
        ...
        if self.getTarget().matches(domainName):
            domevt = SpiderFootEvent(
                "DOMAIN_NAME", domainName, self.__name__, parentEvent
            )
            self.notifyListeners(domevt)
        else:
            ...
            if parentEvent.data.endswith("." + domainName):
                domevt = SpiderFootEvent(
                    "DOMAIN_NAME_PARENT", domainName, self.__name__, parentEvent
                )
                self.notifyListeners(domevt)
