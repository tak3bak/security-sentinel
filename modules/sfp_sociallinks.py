...
class sfp_sociallinks(SpiderFootPlugin):
    ...
    def query(self, queryString):
        ...
        res = self.sf.fetchUrl(
            queryString, headers=headers, timeout=60, useragent=self.opts["_useragent"]
        )
        ...
        if eventName == "PHONE_NUMBER":
            ...
            if resultSet:
                if resultSet.get("first_name") and resultSet.get("last_name"):
                    evt = SpiderFootEvent(
                        "HUMAN_NAME",
                        f"{resultSet.get('first_name')} {resultSet.get('last_name')}",
                        self.__name__,
                        event,
                    )
                ...
                if resultSet.get("username"):
                    evt = SpiderFootEvent(
                        "USERNAME", resultSet.get("username"), self.__name__, event
                    )
                    self.notifyListeners(evt)

                evt = SpiderFootEvent(
                    "RAW_RIR_DATA", str(resultSet), self.__name__, event
                )
                self.notifyListeners(evt)
            ...
            if data is None:
                ...
                if resultSet:
                    ...
                    if resultSet.get("profileurl").get("_content"):
                        evt = SpiderFootEvent(
                            "SOCIAL_MEDIA",
                            f"Flickr: <SFURL>{resultSet.get('profileurl').get('_content')}</SFURL>",
                            self.__name__,
                            event,
                        )
                        self.notifyListeners(evt)
            ...
            if data is None:
                ...
                if resultSet:
                    ...
                    if resultSet.get("skypeId"):
                        evt = SpiderFootEvent(
                            "ACCOUNT_EXTERNAL_OWNED",
                            f"Skype [{resultSet.get('skypeId')}]",
                            self.__name__,
                            event,
                        )
                        self.notifyListeners(evt)
                        evt = SpiderFootEvent(
                            "USERNAME", resultSet.get("skypeId"), self.__name__, event
                        )
                        self.notifyListeners(evt)
            ...
            if data is None:
                ...
                if resultSet:
                    ...
                    if resultSet.get("headline"):
                        evt = SpiderFootEvent(
                            "JOB_TITLE", resultSet.get("headline"), self.__name__, event
                        )
                        self.notifyListeners(evt)

                    evt = SpiderFootEvent(
                        "SOCIAL_MEDIA",
                        f"LinkedIn: <SFURL>{resultSet.get('linkedInUrl')}</SFURL>",
                        self.__name__,
                        event,
                    )
                    self.notifyListeners(evt)
