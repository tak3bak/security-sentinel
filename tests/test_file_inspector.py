...
class TestFileInspector(unittest.TestCase):
    ...
    @patch(
        ...
        read_data="AWS_SECRET_ACCESS_KEY=abc123",
    )
    def test_inspect_file_leak_detection(self, mock_file):
        handler = SecuritySentinelHandler()
        handler.quarantine_file = patch(
            "security_sentinel.file_inspector.SecuritySentinelHandler.quarantine_file"
        ).start()

        handler.inspect_file("test_file.txt", "test_file.txt")

        handler.quarantine_file.assert_called_once_with(
    ...
    @patch(
        "security_sentinel.file_inspector.SecuritySentinelHandler.trigger_spiderfoot"
    )
